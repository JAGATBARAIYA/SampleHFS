...
- (CGFloat)menuWidth {
    if(self.frame.size.width > 0) {
        ...
        return kYSLScrollMenuViewWidth;
    }
}
...
- (void)setItemTitleArray:(NSArray *)itemTitleArray
{
    if (_itemTitleArray != itemTitleArray) {
        ...
        for (int i = 0; i < itemTitleArray.count; i++) {
            CGRect frame = CGRectMake(0, 0, self.menuWidth, CGRectGetHeight(self.frame));
        ...
        _indicatorView.frame = CGRectMake(0, _scrollView.frame.size.height - kYSLIndicatorHeight, self.menuWidth, kYSLIndicatorHeight);
...
- (void)setIndicatorViewFrameWithRatio:(CGFloat)ratio isNextItem:(BOOL)isNextItem toIndex:(NSInteger)toIndex
{
    ...
    if (isNextItem) {
        indicatorX = ((kYSLScrollMenuViewMargin + self.menuWidth) * ratio ) + (toIndex * self.menuWidth) + ((toIndex + 1) * kYSLScrollMenuViewMargin);
    } else {
        indicatorX =  ((kYSLScrollMenuViewMargin + self.menuWidth) * (1 - ratio) ) + (toIndex * self.menuWidth) + ((toIndex + 1) * kYSLScrollMenuViewMargin);
    }
    
    if (indicatorX < kYSLScrollMenuViewMargin || indicatorX > self.scrollView.contentSize.width - (kYSLScrollMenuViewMargin + self.menuWidth)) {
        return;
    }
    _indicatorView.frame = CGRectMake(indicatorX, _scrollView.frame.size.height - kYSLIndicatorHeight, self.menuWidth, kYSLIndicatorHeight);
}

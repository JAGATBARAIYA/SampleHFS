...
- (void)setChartLabelLines {
    
    [_yChartLabels enumerateObjectsUsingBlock:^(PNChartLabel *lbl, NSUInteger idx, BOOL *stop) {
        
        CAShapeLayer *labelLine = [CAShapeLayer layer];
        labelLine.lineCap      = kCALineCapButt;
        labelLine.fillColor    = [[UIColor whiteColor] CGColor];
        labelLine.lineWidth    = 1.0;
        labelLine.strokeEnd    = 0.0;
        
        UIBezierPath *progressline = [UIBezierPath bezierPath];
        
        [progressline moveToPoint:CGPointMake(_chartMargin, lbl.center.y)];
        [progressline addLineToPoint:CGPointMake(self.frame.size.width - _chartMargin,  lbl.center.y)];
        
        [progressline setLineWidth:1.0];
        [progressline setLineCapStyle:kCGLineCapSquare];
        labelLine.path = progressline.CGPath;
        labelLine.strokeColor = PNLightGrey.CGColor;
        ...
        pathAnimation.fromValue = @0.0f;
        pathAnimation.toValue = @1.0f;
        [labelLine addAnimation:pathAnimation forKey:@"strokeEndAnimation"];
        
        labelLine.strokeEnd = 1.0;
        
        [self.layer addSublayer:labelLine];
    }];
}
